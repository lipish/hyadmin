...
def bench_moe(quant_mode: str):
    with torch.inference_mode(mode=True):
        ...
        for ilayer in range(layer_num):
            print(f"layer [{ilayer:02}/{layer_num:02}]")
            ...
            gate_inv = torch.randn((expert_num, intermediate_size // 128, hidden_size // 128), dtype=torch.float32, device = "cuda").to("cpu").contiguous()
            up_inv = torch.randn((expert_num, intermediate_size // 128, hidden_size // 128), dtype=torch.float32, device = "cuda").to("cpu").contiguous()
            down_inv = torch.randn((expert_num, hidden_size // 128, intermediate_size // 128), dtype=torch.float32, device = "cuda").to("cpu").contiguous()

            print("generated")
            config = heyi.moe.MOEConfig(
            ...
            moe = heyi.moe.MOE(config)
            print("loaded")
            gate_projs.append([gate_proj, gate_inv])
            up_projs.append([up_proj, up_inv])
            down_projs.append([down_proj, down_inv])
            moes.append(moe)
            print("done")
