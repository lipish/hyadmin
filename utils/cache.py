...
class StaticPagedCache(transformers.StaticCache):
    """
    ...
    def copy_(self, other: "StaticPagedCache") -> None:
        """
        In-place copy from another StaticPagedCache instance. Both must be of the same structure.
        
        Parameters:
            other (`StaticPagedCache`): The source cache to copy from.
        """
        if not isinstance(other, StaticPagedCache):
            raise TypeError(f"Expected StaticPagedCache, got {type(other)}")
        
        if self.num_hidden_layers != other.num_hidden_layers:
            raise ValueError("Cannot copy_: mismatched number of hidden layers.")
        
        if self.is_MLA != other.is_MLA:
            raise ValueError("Cannot copy_: mismatched MLA mode.")
        
        for idx in range(self.num_hidden_layers):
            self.key_cache[idx].copy_(other.key_cache[idx])
            if self.value_cache[idx] is not None and other.value_cache[idx] is not None:
                self.value_cache[idx].copy_(other.value_cache[idx])
            elif self.value_cache[idx] is not None or other.value_cache[idx] is not None:
                raise ValueError("Cannot copy_: mismatched value_cache presence.")
            
            self.past_tokens[idx] = other.past_tokens[idx]

        if self.is_MLA:
            # Share the same page table references
            for i in range(len(self.page_table_list)):
                self.page_table_list[i].copy_(other.page_table_list[i])
